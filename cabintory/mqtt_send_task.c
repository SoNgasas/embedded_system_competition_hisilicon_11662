...
void basketListToJson(BasketNode *head, cJSON **root) {  
    *root = cJSON_CreateArray();  
    ...
    while (current != NULL) {  
        cJSON *basketObj = cJSON_CreateObject();  
        cJSON_AddStringToObject(basketObj, "item", current->basket.item);  
        cJSON_AddNumberToObject(basketObj, "weight", current->basket.weight); 
        cJSON_AddStringToObject(basketObj,"entrytime",current->basket.datetime);
        cJSON_AddItemToArray(*root, basketObj);  
...
void publish_sensor_data(msg_data_t *msg)
{
    ...
    if (sprintf_s(publish_topic, MQTT_TOPIC_MAX, MQTT_TOPIC_PUB_PROPERTIES, DEVICE_ID) > 0) {
        ...
        cJSON_AddNumberToObject(properties, "totalweight", msg->totalweight);
        cJSON_AddNumberToObject(properties, "temperature", msg->temperature);
        cJSON_AddNumberToObject(properties, "humidity", msg->humidity);

        basketjson=cJSON_CreateArray();
        basketListToJson(basketList, &basketjson); 
        cJSON_AddItemToObject(properties, "basket", basketjson);
